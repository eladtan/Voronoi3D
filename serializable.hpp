...
namespace {
	template<class T> vector<T> chunk(const vector<T>& source, size_t i_start, size_t i_end)
	{
...
vector<double> list_serialize(const vector<Serializable*>& los);

template <class S>
vector<double> list_serialize(const vector<S> los)
{
	if (los.empty())
