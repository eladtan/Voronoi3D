...
namespace
{
	...
	void SetPointTetras(vector<vector<size_t> > &PointTetras, size_t Norg, vector<Tetrahedron> const& tetras,
	...
	/*void BuildBigTetra(Tessellation3D const& tproc, vector<Vector3D> & mesh_points)
	{
		...
		vector<std::size_t> const& faces = tproc.GetCellFaces(static_cast<std::size_t>(rank));
		for (std::size_t i = 0; i < faces.size(); ++i)
		{
			vector<std::size_t> const& fp = tproc.GetPointsInFace(faces[i]);
...
{
	boost::container::flat_map<std::size_t, Face> boundary_faces;
	...
	for (std::size_t i = 0; i < to_duplicate.size(); ++i)
	{
		...
		std::size_t other = (neigh.first == static_cast<std::size_t> (rank)) ? neigh.second : neigh.first;
		if (other < Nproc)
	...
	std::fill(duplicated_points_.begin(), duplicated_points_.end(), vector<std::size_t>());
	vector<Vector3D> res;
	...
	for (std::size_t i = 0; i < to_duplicate.size(); ++i)
	{
		...
		if (other < Nproc)
		{
			std::size_t index = std::lower_bound(duplicatedprocs_.begin(), duplicatedprocs_.end(), static_cast<int>(other))
		...
		{
			if (boundary_faces.find(other) == boundary_faces.end())
			{
				Face f(VectorValues(tproc.GetFacePoints(), tproc.GetPointsInFace(to_duplicate[i].first)), 0, 0);
				boundary_faces.insert(std::pair<std::size_t, Face>(other, f));
			}
			self_duplicate_.push_back(to_duplicate[i].second);
			res.push_back(MirrorPoint(boundary_faces[other], del_.points_[to_duplicate[i].second]));
		}
	}
	// Communicate
	vector<vector<Vector3D> > toadd = MPI_exchange_data(duplicatedprocs_, duplicated_points_, del_.points_);
	// Add points
	Nghost_.clear();
...
void Voronoi3D::Build(vector<Vector3D> const & points, Tessellation3D const& tproc)
{
	...
	del_.points_.reserve(new_points.size() + 4);
	del_.points_ = new_points;
	Norg_ = del_.points_.size();
	// Build big tetrahedron
	BuildBigTetra(tproc, del_.points_);

	tetgenio tetin, tetout;
	RunTetGen(del_.points_, tetin, tetout);
	CopyData(tetout);
	...
	del_.points_.insert(del_.points_.end(), extra_points.begin(), extra_points.end());
	tetin.deinitialize();
	tetin.initialize();
	tetout.deinitialize();
	tetout.initialize();
	...
	tetout.deinitialize();
	tetout.initialize();
	RunTetGen(del_.points_, tetin, tetout, true);
...
void Voronoi3D::Build(vector<Vector3D> const & points)
{
	...
	SetPointTetras(PointTetras_, Norg_, del_.tetras_, del_.empty_tetras_);
...
vector<std::size_t> const& Voronoi3D::GetSelfDuplicate(void)const
{
